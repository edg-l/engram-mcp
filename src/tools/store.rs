...
///
/// Two kinds of candidate are refused even when similar enough:
///
/// - anything in `exempt`, which the caller has declared distinct from the new memory
///   (its `related_to` and `supersedes` targets);
/// - anything that is already a merge composite. Merging into a memory that has already
///   absorbed others is how claims with different lifetimes end up in one record, which
///   is exactly what a later delete cannot verify claim by claim.
pub(crate) fn find_duplicates(
    ...
    for (existing_id, existing_vec) in existing_embeddings {
        if exempt.contains(existing_id) {
            continue;
        }
        ...
        if similarity >= threshold
            ...
            && mem.merged_from.as_ref().is_none_or(|s| s.is_empty())
        {
...
#[allow(dead_code)] // Used by the hook store path and external callers
...
/// [`store_with_dedup`], refusing to merge the new memory into any id in `exempt`.
///
/// Callers pass the memories they have explicitly related to or superseded: having said
/// "this is connected to that, and different from it", they should not then have the two
/// collapsed into one record.
#[allow(clippy::too_many_arguments)]
pub fn store_with_dedup_exempting(
    db: &Database,
    embedding_service: Option<&EmbeddingService>,
    project_id: &str,
    memory: Memory,
