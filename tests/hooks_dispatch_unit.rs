...
fn dispatch_user_prompt_submit_no_cue_skipped() {
    let (_dir, db) = fresh_db();
    // 71 chars, no cue word from the default regex
...
fn dispatch_subagent_stop_disabled_by_default() {
    // Ensure no leftover env override from parallel tests.
    // SAFETY: single-threaded env mutation; other tests do not set this var.
    unsafe {
