...
fn test_store_and_query_realistic_memories() {
    ...
    let r = call(&h, "memory_query", json!({
    ...
    let memories = r["memories"].as_array().unwrap();
    assert!(!memories.is_empty());
    let contents: Vec<&str> = memories.iter()
    ...
    let r = call(&h, "memory_context", json!({
    ...
    // Update a memory
    let r = call(&h, "memory_update", json!({
        "id": api_id,
...
fn test_dedup_triggers_on_rephrased_memory() {
    ...
    // Agent stores a fact
    let r1 = call(&h, "memory_store", json!({
    ...
    let r2 = call(&h, "memory_store", json!({
    ...
    if r2.get("merge_info").is_some() && !r2["merge_info"].is_null() {
        ...
        let ids: Vec<&str> = r["memories"].as_array().unwrap().iter()
            .map(|m| m["memory"]["id"].as_str().unwrap())
            .collect();
        assert!(!ids.contains(&id1.as_str()), "Old memory should be merged away");
...
fn test_bulk_dedup_tool() {
    ...
    let before_count = call(&h, "memory_stats", json!({}))["memory_count"].as_u64().unwrap();

    // Memory count unchanged after dry run
    let after_count = call(&h, "memory_stats", json!({}))["memory_count"].as_u64().unwrap();
...
fn test_branch_scoped_memories() {
    ...
    let branch_memories: Vec<&Value> = r["memories"].as_array().unwrap().iter()
...
fn test_clustering_forms_naturally() {
    ...
    assert!(cluster_count >= 1, "Expected clusters to form, got {}", cluster_count);
...
fn test_relationship_graph_and_links() {
    ...
    let r3 = call(&h, "memory_store", json!({
    ...
    let r = call(&h, "memory_graph", json!({
        "id": decision_id,
        "depth": 2
    }));
...
fn test_batch_store_and_export_import() {
    ...
    let r = call(&h, "memory_store_batch", json!({
        "memories": [
            {
    ...
    let r = call(&h2, "memory_import", json!({
        "data": r,
        "mode": "merge"
    }));
    assert!(r["success"].as_bool().unwrap());
    ...
    let r = call(&h2, "memory_query", json!({
    ...
    let contents: String = results.iter()
        ...
        .join(" ");
    assert!(
        contents.contains("OutOfMemory") || contents.contains("payment") || contents.contains("channel"),
...
fn test_prune_low_relevance_memories() {
    ...
    let r = call(&h, "memory_prune", json!({
        "threshold": 0.99
    }));
    assert!(r["dry_run"].as_bool().unwrap());
    ...
    assert_eq!(candidates, 0, "Fresh memories should all have relevance 1.0");
